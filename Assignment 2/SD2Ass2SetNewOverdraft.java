...
public class SD2Ass2SetNewOverdraft extends JFrame implements ActionListener
{
	//create GUI components
	private JTextField idField, firstNameField, lastNameField, balanceField, overDraftField;;
	private JButton setNewOverDr, exitBut;
	private RandomAccessFile input, output;
	private SD2Ass2Record data;

	public SD2Ass2SetNewOverdraft()
	{
		super ("Set New Overdraft");
		...
		setNewOverDr = new JButton( "Set New Overdraft Limit");
		setNewOverDr.addActionListener( this );
		add(setNewOverDr);
		setNewOverDr.setEnabled(false);
	...
	public void actionPerformed( ActionEvent e )
	{
		...
		if (e.getSource() == setNewOverDr)
		{
			try
			{
				...
				overdraft = Integer.parseInt(JOptionPane.showInputDialog("Enter New Overdraft"));

				data.setOverDraftLimit( overdraft );

				output.seek( (long) ( accId-1 ) * SD2Ass2Record.size() );
				data.write( output );

				JOptionPane.showMessageDialog(this, "Overdraft is changed to   € " + overdraft+ "\n            (Account ID " + data.getAccountID() + ")" );
